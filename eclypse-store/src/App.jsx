  ...
  return (
    <div className="min-h-screen bg-background text-foreground transition-colors duration-300">
      <header className="container mx-auto px-4 py-6 flex justify-between items-center border-b border-border" role="banner">
        <h1 className="text-3xl font-bold"><a href="/" aria-label="Página inicial da Eclypse">Eclypse</a></h1>
            ...
            <SheetContent className="w-full sm:w-[400px] flex flex-col" role="dialog" aria-modal="true" aria-label="Carrinho de Compras">
                  ...
                  <ul className="space-y-4" role="list">
                    {cartItems.map((item) => (
                      <li key={item.id} className="flex items-center space-x-4" role="listitem">
                        <img src={item.image} alt={`Imagem do produto ${item.name}`} className="w-16 h-16 object-cover rounded-md" />
              ...
              {cartItems.length > 0 && (
                <div className="border-t border-border pt-4">
                  ...
                  <Button className="w-full mt-4" aria-label="Finalizar Compra">Finalizar Compra</Button>
                </div>
              )}
            </SheetContent>
          </Sheet>
